...
<template>
	...
	>
		...
		>
			<a href="#hero" class="z-50">
				<img 
					:class="[
						isSticky 
							? 'w-16 lg:w-20 mx-2 my-1' 
							: 'max-lg:w-20 max-lg:mx-4 max-lg:my-2',
		...
		<div class="flex items-center gap-4 transition-all duration-500 ease-in-out">
			...
			<button 
				@click="toggleMenu" 
				class="lg:hidden text-white z-50 focus:outline-none transition-all duration-300 hover:scale-110 active:scale-95"
			>
				<svg 
					v-if="!isMenuOpen" 
				...
				<svg 
					v-else 
					xmlns="http://www.w3.org/2000/svg" 
					class="h-8 w-8 transition-transform duration-300 rotate-90" 
					fill="none" 
					viewBox="0 0 24 24"
					stroke="currentColor"
				>
	...
	>
		<div 
			v-if="isMenuOpen"
			class="fixed inset-0 bg-dark/95 backdrop-blur-sm z-40 flex flex-col items-center justify-center lg:hidden"
		>
