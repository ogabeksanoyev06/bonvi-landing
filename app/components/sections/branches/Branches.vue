...
const focusOnBranch = (branch: any) => {
	...
	if (lat && lng) {
		...
		if (targetMarker) {
			targetMarker.options.set('iconImageHref', 'data:image/svg+xml;base64,' + createMarkerSVG('#0082FF'))
				...
				branchesData.value.forEach((branch: any, index: number) => {
					...
					if (lat && lng) {
						...
						customMarker.events.add('click', () => {
							...
							customMarker.options.set('iconImageHref', 'data:image/svg+xml;base64,' + createMarkerSVG('#0082FF'))
