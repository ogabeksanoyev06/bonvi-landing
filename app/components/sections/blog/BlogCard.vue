<template>
	<div class="relative z-10 overflow-hidden rounded-3xl md:rounded-[48px] p-4 md:p-8 lg:p-10 min-h-[300px] md:min-h-[400px] flex flex-col items-start bg-orange-600">
		...
		<div class="flex flex-col gap-3">
			<div class="px-3 py-1 bg-dark rounded-full">
				<span class="text-xl md:text-2xl lg:text-[32px] text-white !leading-130">{{ formatMoneyDecimal(price) }} </span>
			</div>
			<span v-if="oldPrice" class="text-lg md:text-2xl !leading-130 line-through text-white">{{ formatMoneyDecimal(oldPrice) }}</span>
		</div>

		<p class="text-xl md:text-2xl font-extrabold !leading-130 text-white font-adero-trial mt-auto">{{ description }}</p>
	</div>
</template>
